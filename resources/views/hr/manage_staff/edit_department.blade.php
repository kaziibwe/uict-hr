<x-layout_hr>
    <div class="mt-10 mb-20 relative overflow-x-auto custom-scrollbar shadow-md sm:rounded-lg z-0">
            ...
            <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="default-tab"
                ...
                <li class="me-2" role="presentation">
                    <button
                        ...
                        data-tabs-target="#staff_member_preview" type="button" role="tab"
                        aria-controls="staff_member_preview" aria-selected="false">
                        Staff Members
                    </button>
                </li>
            </a>
        ...
        <div class="p-4 rounded-lg bg-gray-50 dark:bg-gray-800" id="department" role="tabpanel"
            ...
            <form action="{{ route('updateDepartment',$department->id) }}" method="post" enctype="multipart/form-data" >
                @method('PUT')
                @csrf
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    <div>
                        <label for="department_name"
                            ...
                            Name</label>
                        <input type="text" id="department_name" name="name"  value="{{ $department->name }}"
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" value="{{ $department->description }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="A Brief sentence on what the department does..." rows="3">{{ $department->description }}</textarea>
                            @error('name')
                            <code class="text-red-800">{{ $message }}</code>
                            @enderror
                    ...
                    <select name="stuff_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" aria-label="Default select example">
                        <option value="">Select HOD</option>
                        @unless ($staffs->isEmpty())
                            @foreach ($staffs as $staff)
                                <option value="{{ $staff->id }}" @if(old('stuff_id') == $staff->id || $department->stuff_id == $staff->id) selected @endif>{{ $staff->name }}</option>
                            @endforeach
                        @else
                            <option value="">No staffs Found</option>
                        @endunless
                    </select>
                    @error('stuff_id')
                        <code>{{ $message }}</code>
            ...
            <div class="flex flex-col items-center">
                ...
                {{-- <span class="text-sm text-gray-700 dark:text-gray-400">
                                    ...
                                    of
                                    <span class="font-semibold text-gray-900 dark:text-white total-staffs"></span>
                                    Entries
                                </span>
